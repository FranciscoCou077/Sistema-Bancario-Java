...
public class CuentaBasica {
    ...
    // Aquí declaramos la estructura de datos para el historial
    ...
    public void depositar(double cantidad) {
        if (cantidad > 0) {
            saldo += cantidad;
            // Guarda el movimiento al principio de la lista (LIFO)
    ...
    public boolean retirar(double cantidad) {
        if (cantidad > 0 && saldo >= cantidad) {
            saldo -= cantidad;
            // Guardamos el retiro al principio de la lista
    ...
    // Método para imprimir el Estado de Cuenta
